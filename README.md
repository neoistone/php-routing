...
implementation works and why it is fast.][Author]
...
```sh
composer require neoistone/php-router
```
...
$dispatcher = Neoistone\simpleDispatcher(function(Neoistone\RouteCollector $r) {
...
switch ($routeInfo[0]) {
    case Neoistone\Dispatcher::NOT_FOUND:
        // ... 404 Not Found
        break;
    case Neoistone\Dispatcher::METHOD_NOT_ALLOWED:
        $allowedMethods = $routeInfo[1];
        // ... 405 Method Not Allowed
        break;
    case Neoistone\Dispatcher::FOUND:
        $handler = $routeInfo[1];
        $vars = $routeInfo[2];
...
The routes are defined by calling the `Neoistone\simpleDispatcher()` function, which accepts
a callable taking a `Neoistone\RouteCollector` instance. The routes are added by calling
...
class name or any other kind of data you wish to associate with the route. Neoistone only tells you
...
$dispatcher = Neoistone\cachedDispatcher(function(Neoistone\RouteCollector $r) {
...
the supplied URI. For example:
    ...
    [Neoistone\Dispatcher::METHOD_NOT_ALLOWED, ['GET', 'POST']]

> **NOTE:** The HTTP specification requires that a `405 Method Not Allowed` response include the
`Allow:` header to detail available methods for the requested resource. Applications using Neoistone
...
and the third array element is a dictionary of placeholder names to their values. For example:
    ...
    [Neoistone\Dispatcher::FOUND, 'handler0', ['name' => 'nikic', 'id' => '42']]
...
$dispatcher = Neoistone\simpleDispatcher(function(Neoistone\RouteCollector $r) {
    ...
    'routeParser' => 'Neoistone\\RouteParser\\Std',
    'dataGenerator' => 'Neoistone\\DataGenerator\\GroupCountBased',
    'dispatcher' => 'Neoistone\\Dispatcher\\GroupCountBased',
]);
```
...
However, implementers using Neoistone outside the web SAPI environment (e.g. a custom server) MUST
NOT send entity bodies generated in response to HEAD requests. If you are a non-SAPI user this is
*your responsibility*; Neoistone has no purview to prevent you from breaking HTTP in such cases.
...
All copyright reserved by [Neoistone][neoistone] and [Nikic][nikic]
